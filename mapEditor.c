/*so sorry some chinese comments here, i will change later*/
#in*clude <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <float.h>
...
int ask_modify_map(AdjacencyList* adj_list)
{
    ...
    printf("\n当前地图中共有 %d 个点和 %d 条边，您已经对地图进行编辑 %d 次。\n",
        adj_list->num_points, adj_list->num_edges, adj_list->num_edit);
    ...
    { /* 提示用户是否需要修改地图属性 */
        ...
        if (strcmp(line, "1") == 0)
        {
            if (edit_map(adj_list) == EDIT_SUCCESSFULLY) 
            {
                modify = 1;
                adj_list->num_edit++;
            }
...
int edit_map(AdjacencyList* adj_list)
{
    ...
    switch (operate_type) 
    {
        case 1:
            ...
            // 查看点是否存在
            int result = is_point_exist(adj_list, point_id, lat, lon, 1);
            ...
            add_point_to_adjacency_list(adj_list, point_id, lat, lon);
        ...
        case 2:
            ...
            while (!flag) // 循环读取输入
            {
                ...
                // 判断两个点是否存在
                if (is_node_exist(adj_list, node_id_from) == NOT_EXIST)
                {
                    printf("The node_id_1 is not exist! please retype!\n");
                    continue;
                }
                if (is_node_exist(adj_list, node_id_to) == NOT_EXIST)
                {
                ...
                if (is_edge_exist(adj_list, link_id, node_id_from, node_id_to, 1) == EXIST)
                {
                    continue;
                }
        ...
        case 3:
            ...
            while (!flag) // 循环读取输入
            {
                ...
                // 判断边 link_id 是否已经存在
                if (is_edge_id_exist(adj_list, link_id) == NOT_EXIST)
                {
            ...
            while (!flag) // 循环读取输入
            {
                ...
                if (is_edge_nodes_exist(adj_list, link_id, node_id_from, node_id_to) == EXIST)
                {
            ...
            // 编辑边
            edit_edge(adj_list, link_id, node_id_from, node_id_to, length);
            printf("已成功添加新边 id = %lld, node_1 <=> node_2 = %lld <=> %lld, length = %lf\n", link_id, node_id_from, node_id_to, length);
...
{
    /* 查找边在邻接表里面的位置 */
    for (int i = 0; i < adj_list->num_edges; i++)
    {
        Edge* edge = &adj_list->edges[i];
        /* 检查空指针 */
        if (edge == NULL)
...
int is_point_exist(AdjacencyList* adj_list, long long point_id, double lat, double lon, int flag) 
{
    ...
    for (int i = 0; i < adj_list->num_points; i++) 
    {
        Point* point = &adj_list->points[i];
        /* 检查空指针 */
        if (point == NULL)
        { /* free memory and exit with NULL_POINTER*/
            free_adjacency_list(adj_list);
...
int is_edge_id_exist(AdjacencyList* adj_list, long long link_id)
{
    ...
    /* 判断边是否已经存在于邻接表中 */
    for (int i = 0; i < adj_list->num_edges - 1; i++)
    {
        Edge* edge = &adj_list->edges[i];
        if (edge->id == link_id)
        {
...
int is_node_exist(AdjacencyList* adj_list, long long node_id)
{
    /* 判断点是否已经存在于邻接表中 边部分*/
    for (int i = 0; i < adj_list->num_points; i++)
    {
